# External Resources

This document contains links to external resources, documentation, and references relevant to the AI Brainstorm project.

## Technology Documentation

### Frontend

- [React Documentation](https://react.dev/)
- [Vite Documentation](https://vitejs.dev/guide/)
- [TailwindCSS Documentation](https://tailwindcss.com/docs)
- [DaisyUI Documentation](https://daisyui.com/docs/)
- [Zustand Documentation](https://github.com/pmndrs/zustand)
- [React Query Documentation](https://tanstack.com/query/latest/docs/react/overview)
- [React Hook Form Documentation](https://react-hook-form.com/get-started)
- [React Router Documentation](https://reactrouter.com/en/main)

### Backend

- [Cloudflare Workers Documentation](https://developers.cloudflare.com/workers/)
- [Hono Documentation](https://hono.dev)
- [Zod Documentation](https://zod.dev)
- [Prisma Documentation](https://www.prisma.io/docs)
- [PostgreSQL Documentation](https://www.postgresql.org/docs/)

### AI Integration

- [OpenAI API Documentation](https://platform.openai.com/docs/api-reference)
- [OpenAI Cookbook (Examples)](https://github.com/openai/openai-cookbook)
- [OpenAI Rate Limits Documentation](https://platform.openai.com/docs/guides/rate-limits)

### Tools & Infrastructure

- [PNPM Documentation](https://pnpm.io/motivation)
- [TypeScript Documentation](https://www.typescriptlang.org/docs/)
- [ESLint Documentation](https://eslint.org/docs/latest/)
- [Prettier Documentation](https://prettier.io/docs/en/)
- [Jest Documentation](https://jestjs.io/docs/getting-started)
- [Testing Library Documentation](https://testing-library.com/docs/)

## Tutorials and Guides

### React & Frontend

- [React with TypeScript Cheatsheet](https://github.com/typescript-cheatsheets/react)
- [Building a Monorepo with PNPM](https://blog.logrocket.com/build-monorepo-next-js/)
- [React Query Data Fetching Guide](https://tkdodo.eu/blog/react-query-data-transformations)
- [Zustand Best Practices](https://docs.pmnd.rs/zustand/guides/practice-with-no-providers)
- [Advanced TypeScript Patterns](https://www.typescriptlang.org/docs/handbook/2/types-from-types.html)

### Cloudflare & Backend

- [Getting Started with Cloudflare Workers](https://developers.cloudflare.com/workers/get-started/guide/)
- [Hono on Cloudflare Workers](https://hono.dev/getting-started/cloudflare-workers)
- [Using Prisma with Cloudflare Workers](https://www.prisma.io/docs/guides/deployment/deployment-guides/deploying-to-cloudflare-workers)
- [API Design Best Practices](https://docs.microsoft.com/en-us/azure/architecture/best-practices/api-design)

### AI & Brainstorming

- [OpenAI Chat Completions Guide](https://platform.openai.com/docs/guides/chat)
- [Prompt Engineering Guide](https://www.promptingguide.ai/)
- [Effective Brainstorming Techniques](https://www.mindtools.com/brainstm.html)
- [Token Usage Optimization](https://platform.openai.com/tokenizer)

## Design Resources

- [DaisyUI Component Examples](https://daisyui.com/components/)
- [Tailwind UI Patterns](https://tailwindui.com/components)
- [Color Palette Generator](https://coolors.co/)
- [UI Design Principles](https://www.interaction-design.org/literature/topics/ui-design-patterns)
- [Accessibility Guidelines (WCAG)](https://www.w3.org/WAI/standards-guidelines/wcag/)

## Performance and Optimization

- [Web Vitals](https://web.dev/vitals/)
- [React Performance Optimization](https://reactjs.org/docs/optimizing-performance.html)
- [Cloudflare Workers Performance](https://developers.cloudflare.com/workers/platform/limits/)
- [API Rate Limiting Best Practices](https://nordicapis.com/everything-you-need-to-know-about-api-rate-limiting/)

## Community and Forums

- [React Dev Community](https://react.dev/community)
- [Cloudflare Workers Discord](https://discord.com/invite/cloudflaredev)
- [Prisma Slack Community](https://slack.prisma.io/)
- [OpenAI Developer Forum](https://community.openai.com/)

## Books and In-depth Learning

- [Patterns.dev](https://www.patterns.dev/) - Modern Web App Design Patterns
- [Frontend Developer Roadmap](https://roadmap.sh/frontend)
- [Fullstack Serverless](https://www.fullstackserverless.dev/)
- [API Design Patterns](https://www.manning.com/books/api-design-patterns)

---

*Last updated: 2023-08-28* 